            ...
            if len(self.person_list) > 0:
                ...
                if len(batch) > 0:
                    ...
                    for emb in scores:
                        ...
                        print(f"Max similarity (confidence): {max_similarity}")
                        if len(data) > 0:
                            index = [*data][0]
                            name = known_embeddings[index]['name']
                            id = known_embeddings[index]['department_id']
                            area = known_embeddings[index]['department_area_id']

                            # Check if the person is whitelisted for any camera
                            is_whitelisted = False
                            for entry in known_embeddings.values():
                                if entry['name'] == name and entry['cam_id'] == "cam" + camid:
                                    is_whitelisted = True
                                    break
                            ...
                            if len(names) > i and len(names[i]) > 2:
                                ...
                                if person_name != "unclear":
                                    if p.firstTime or p.firstTimeName:
                                        ...
                                        sql = "INSERT INTO detected_frames (cam, filepath, findings, personname, datetime, department_id, department_area_id) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                                        val = (setcamid, filename, findings, person_name, getdatetime, id, area)
...
class EmbeddingHandler:
    def __init__(self, output_path=None):
        ...
        self.embeddings_file = os.path.join(self.output_path, "newpk3.pkl")
    ...
    def update_embeddings(self):
        ...
        for row in rv:
            ...
            if not filepath:
                # If the filepath is not found for the current person_id, search for the same person with a different cam_id
                sql_select_query = """SELECT whitelisted_pictures.filepath FROM whitelisted_pictures
                                      INNER JOIN person_info ON whitelisted_pictures.person_id = person_info.id
                                      WHERE person_info.name = %s AND person_info.cam_id != %s
                                      LIMIT 1"""
                input_data = (name, cam_id)
                cursor.execute(sql_select_query, input_data)
                rv = cursor.fetchall()
                for row in rv:
                    filepath = row[0]

            # Select the department id for the corresponding cam id
            sql_select_query = """SELECT camera_info.department_id FROM person_info 
                                  INNER JOIN camera_info ON person_info.cam_id = camera_info.cam_id 
                                  WHERE person_info.name = %s"""
            input_data = (name,)
            cursor.execute(sql_select_query, input_data)
            rv = cursor.fetchall()
            department_id = None
            for row in rv:
                department_id = row[0]
                # Select the department id for the corresponding cam id
            sql_select_query = """SELECT camera_info.department_area_id FROM person_info 
                                              INNER JOIN camera_info ON person_info.cam_id = camera_info.cam_id 
                                              WHERE person_info.name = %s"""
            input_data = (name,)
            cursor.execute(sql_select_query, input_data)
            rv = cursor.fetchall()
            department_area_id = None
            for row in rv:
                 department_area_id = row[0]

            if filepath and department_id is not None:
                embededFile = getEmbed(
                    file="D:/xampp/htdocs/emacsPanel/detected_pictures/uploaded/" + filepath.replace('\\', '/'))
                index = len(embeddings)
                embeddings[index] = {'name': name, 'cam_id': cam_id, 'department_id': department_id,'department_area_id':department_area_id,
                                     'embedding': embededFile}
                # print(embeddings)
