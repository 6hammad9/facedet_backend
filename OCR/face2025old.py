    ...
    def extract_text(self, image, bboxes, bboxes_p, tracker, camid):
        ...
        try:
            if len(self.person_list) > 0:
                ...
                if len(batch) > 0:
                    pred = ort_sess_r.run(None, {'input.1': batch})  # pred[0] = list of embeddings
                    names = []

                    for face_embedding in pred[0]:
                        if users:
                            name, similarity = find_best_match(face_embedding, users)
                            print(f"[MATCH] Best match: {name} with similarity: {similarity}")
                        else:
...
def generate_encodings_from_folder(WHITELIST_FOLDER, USERS_PKL_PATH):
    ...
    for path in image_paths:
        try:
            ...
            embedding = getEmbed(path)  # assign to variable
            print(f"[DEBUG] Saving {name}, embedding shape: {embedding.shape}, type: {type(embedding)}")
